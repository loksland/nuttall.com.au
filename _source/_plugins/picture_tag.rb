    ...
    def render(context)
      ...
      instance.each { |key, source|
        instance[key][:generated_src] = generate_image(source, site.source, site.dest, settings['source'], settings['output'], site.config["baseurl"])
			...
			source_height = 0
			instance.each { |key, source|
				image = MiniMagick::Image.open(File.join(site.source, settings['source'], source[:src]));
      ...
      if settings['markup'] == 'picture'
        ...
        picture_tag = "<picture data-src-w=\"#{source_width.round}\" data-src-h=\"#{source_height.round}\" >\n"\
                      "#{source_tags}"\
